(function ($) {
    ...
    function ElementSet($this, AttrName, name, value) {
        ...
        if ($taget.length > 0) {
            if ($taget.attr('type') == "date") {
                var date = new Date(value.replace(new RegExp(/-/gm), "/"));
                var datestr = date.getFullYear() + "-" + ("0" + (date.getMonth() + 1)).slice(-2) + "-" + ("0" + date.getDate()).slice(-2);
                $taget.val(datestr);
            }
            else if ($taget.attr('type') == "datetime") {
                var datetime = new Date(value.replace(new RegExp(/-/gm), "/"));
                var datetimestr = datetime.getFullYear() + "-" + ("0" + (datetime.getMonth() + 1)).slice(-2) + "-" + ("0" + datetime.getDate()).slice(-2) + " " + ("0" + datetime.getHours()).slice(-2) + ":" + ("0" + datetime.getMinutes()).slice(-2) + ":" + ("0" + datetime.getSeconds()).slice(-2);
                $taget.val(datetimestr);
            }
            else if ($taget.attr('type') == "datetime-local") {
                $taget.val(value);
            }
            else {
                $taget.val(value);
